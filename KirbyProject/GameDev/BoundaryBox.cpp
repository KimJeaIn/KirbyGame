...
BYTE SquareBox::BoxIsCollide(BoundaryBox* other, const bool& back)
{
   ...
   if (other->type() == BBOX_SQUARE)
   {
	  ...
	  {
		  if(rcBox.bottom > oBox.top && rcBox.top+30 < oBox.top)
		  {
			  if(rcBox.left < oBox.left)
				  return 20;
			  if(rcBox.right > oBox.right)
				  return 10;
			  return 0;
		  }
		  else if(rcBox.top < oBox.bottom && rcBox.bottom > oBox.bottom)
		  {
			  return 3;
		  }
		  else if(rcBox.left < oBox.right && rcBox.right > oBox.right && back)
		  {
			  return 1;
		  }

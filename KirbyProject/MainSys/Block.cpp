...
BOOL Block::Load(LPCTSTR szFileName, const Rect& rc)
{
   ...
   // Bitmap 창고에 읽을 이미지 파일이 없다면... 읽도록 한다.
   if (!BitmapDepot.get(szFileName))
   {
	   // 지정된 이미지를 읽어들인다.
...
BOOL Block::Load(LPCTSTR szFileName)
{
   ...
   BitmapSize = Size(bm.bmWidth, bm.bmHeight);
...
void Block::release()
{
	// Bitmap 창고에서 관리하므로 여기서는 제거하지 않는다.
...
void Block::BoxGap(const LONG& ht, const LONG& lt, const LONG& rt, const LONG& bt)
{
	Gap = ht;
	GapL = lt;
	GapR = rt;
